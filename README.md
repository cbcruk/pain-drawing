...
- **무릎은 전부 모식도다.** `refs/`에 무릎 도판이 없다. 앞 21개 구조 · 뒤 15개
  배치 모두 손으로 찍었고, 뷰가 `schematic`이라 푸터도 그렇게 말한다. 뒤 뷰의
  기하는 앞 뷰를 좌우 반전한 것이고(x' = 320 − x), 관절 **안**의 구조는 좌표까지
  반전값이다 — 같은 무릎을 반대에서 본 것이라 전두면 위치가 거울상이기 때문이다.
  표층(햄스트링건·비복근두)과 후방인대만 새로 찍었다.
