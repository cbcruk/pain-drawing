...
src/
  ...
  data/
    ...
    lower-leg/
      structures.ts             종아리에만 있는 20개. 경계를 넘는 것은 id로 공유
...
- [x] **종아리** — 첫 확대 부위. 뒤·바깥·안쪽 세 면, placement 84개가 **전부
      `reachable: true`**다. 부위 경계를 넘는 구조를 id로 공유해 `Structure`가
      부위가 아니라 구조 하나에 붙는다는 것을 처음 검증했다
...
- **안쪽 뷰는 통증 위치 때문에 넣었다.** 경골 내측면은 근육이 안 덮어 뼈를 직접
  ...
  규칙이 없으면 표현할 수 없다.
- **종아리는 전부 모식도다.** 구조 20개를 새로 만들고 배치 84개(좌우 포함)를
