...
- **정밀도 주장도 StructureInView가 할 수 있다.** 도판이 층별이라 트레이싱도
  ...
  종아리가 그 상태로 간다 — Gray's에 비복근을 온전히 그린 도판도, 다리 외측면
  도판도 없으므로 그 자리들은 뷰가 올라가도 `fidelity: 'schematic'`을 **명시**해
  상속을 끊는다.
- **등급은 셋이다 — `schematic` < `normalized` < `traced`.** 가운데가 있는 이유는
  자료와 뷰의 **비율**이 다를 때 상사변환이 존재하지 않기 때문이다. 그때는 좌표
  대신 "어느 높이에서 폭의 몇 지점인가"만 옮긴다(`lib/normalize.ts`). 순서와
  상대 위치는 살아남고 모양과 각도는 살아남지 않는다. 근거는 SPEC "비율이 다른
  자료 — 정규화 대응".
...
- **depth는 Structure가 아니라 StructureInView의 속성이다.** 같은 구조가 뷰마다
  ...
  종아리 가로단면(Gray440)이 이걸 자료로 증명한다: 같은 경골신경이 뒤 뷰에서는
  심부 굴근보다 **얕고**(L3에서 굴근 위) 안쪽 뷰에서는 **더 깊다**(L2 굴근 밑
  L3). 둘 다 맞다 — 깊이는 절대 위치가 아니라 그 뷰가 들어가는 **방향으로 잰
  거리**이기 때문이다.
- **이름의 근거는 `ta`가 댄다.** `name`의 네 이름은 전부 우리가 적은 것이라
  스스로 검증되지 않는다. `ta`는 국제 표준(TA98)에 그 코드로 실린 항목이 있고
  라틴어 이름이 우리 `la`와 같다는 대조 결과다. **대응 항목이 없으면 비운다** —
  표준이 우리보다 잘게 나눈 자리(비골근지대는 상·하 둘)에서 하나를 골라 적으면
  그 레코드가 아닌 것을 가리키게 된다. 근거는 SPEC "표준 용어 대조".
