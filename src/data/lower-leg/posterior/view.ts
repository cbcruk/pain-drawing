...
export const lowerLegPosteriorView: View = {
  ...
  */
  layers: [
    ...
    /*
      신경·혈관을 이름에 함께 적는 이유는 Gray440(다리 중간 가로단면)이다.
      경골신경과 후경골동맥은 심부 굴근과 **같은 깊이가 아니라** 가자미근과
      굴근 사이에 얹혀 있다. 뒤에서 벗겨 들어가면 가자미근을 젖히는 순간 굴근
      무리 위에 놓인 채로 함께 드러나므로 같은 L3가 맞지만, 층 이름이 "심부
      굴근"이면 그 둘을 굴근이라고 말하게 된다. 도형을 옮기지 않고 이름을
      고치는 것은 안쪽 뷰 L3를 "족근관"에서 "심부 통로"로 바꿨을 때와 같은
      판단이다 — 틀린 건 그림이 아니라 라벨이었다.
    */
    {
      depth: 3,
      ko: '심부 굴근 · 신경혈관',
      en: 'deep flexors & neurovascular bundle',
      hint: '뼈에 붙어 발가락을 당기는 근육과 그 위를 지나는 신경·혈관',
    },
  ],
}
