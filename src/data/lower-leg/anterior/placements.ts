...
/*
  ...
  옮기고 나서 재보니 6개가 윤곽 밖에 있었다. 정규화가 상대 위치를 보존하므로
  **이번에 생긴 게 아니라 원래 밖에 있던 것**이 그대로 따라온 것이다
  (제3비골근 18.8 → 18.5).

  원인은 복사뼈 랜드마크가 다리 축에서 13만큼 바깥으로 밀려 있었다는 것이고,
  그 자리에 맞춰 그려졌으니 함께 밀려 있었다. 랜드마크를 축에 맞춘 뒤 이
  6개도 여백 2를 두고 안으로 들였다. 폭은 건드리지 않았다 — 자리만 틀렸지
  굵기는 틀린 게 아니다.

  이건 트레이싱이 아니라 **도형이 다리 밖에 있으면 안 된다**는 기하 교정이라
  등급이 모식도 그대로다.
*/
export const lowerLegAnteriorPlacements: StructureInView[] = [
  {
    ...
    shapes: [
      {
        t: 'ribbon',
        p: [[161.8, 45], [161.99, 200], [162.05, 360], [161, 500], [159.6, 570]],
  ...
  },
  {
    ...
    fidelity: 'schematic',
    shapes: [
      { t: 'ribbon', p: [[143.29, 592], [161, 584], [178.72, 592]], w: [29.57, 33.03, 29.57] },
    ],
  },
  ...
  },
  {
    ...
    shapes: [
      {
        t: 'ribbon',
        p: [[134.2, 460], [137.59, 520], [139.64, 570], [137.17, 620], [136.54, 662]],
  ...
  },
  {
    ...
    shapes: [
      {
        t: 'ribbon',
        p: [[124.92, 440], [132.4, 500], [136.07, 552], [134.17, 600]],
  ...
  },
  {
    ...
    shapes: [
      {
        t: 'ribbon',
        p: [[107.89, 300], [110.95, 380], [121.02, 450], [128.85, 510], [132.46, 560]],
  ...
  },
  {
    ...
    shapes: [
      ...
      {
        t: 'ribbon',
        p: [[188.82, 600], [190.73, 520], [204.82, 420], [213.93, 310], [213.61, 200], [206.06, 80]],
