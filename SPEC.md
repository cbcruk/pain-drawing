...
### 표준 용어 대조 — `Structure.ta`

`name`의 네 이름은 전부 **우리가 적은 것**이라 스스로 검증되지 않는다. 좌표는
도판이라는 밖의 기준을 두고 `fidelity`로 그 주장을 표시하는데, 이름에는 그런
기준이 없었다. `ta`가 그 자리다 — 국제 해부학 용어 표준에 이 코드로 실린 항목이
있고 그 라틴어 이름이 우리 `la`와 같다는, **대조한 결과**다.

판(`edition`)을 함께 적는다. TA98은 `A04.7.02.047` 형태이고 TA2는 일련번호라,
문자열 하나로 두면 나중에 어느 판인지 알 수 없다. `fmaId`가 옆에 남아 있는
이유는 대조에 쓴 데이터셋이 둘을 함께 담고 있기 때문이다.

**대조 방법.** Open Anatomy Project의 TAViewer(`mhalle/taviewer`, MIT)에 들어
있는 `src/human.json`이 TA98 코드 · 라틴어 · 영어 · FMA id를 트리로 담고 있다.
라틴어 이름만으로 기계 대조했고, 표준이 `m. soleus`처럼 줄여 쓰므로
`m.`·`n.`·`v.`·`lig.` 따위를 풀어 비교했다. 계층 이름도 푼다 — TA98은 부모 밑에서
`caput mediale`이라고만 쓰고 우리 목록은 평평해서 `caput mediale musculi
gastrocnemii`로 적으므로, 자식 이름 + 부모 이름으로 맞춘다.

**세 부위 72개 중 54개가 붙었다.** 종아리 22/23 · 발 17/28 · 무릎 15/21.

**비우는 것도 결과다.** 대응 항목이 없으면 근처 코드를 끌어다 적지 않는다. 그러면
그게 나중에 근거로 둔갑한다 — 도판 번호를 캡션으로 확정하는 규칙과 같다. 빈 18개는
네 종류로 갈린다.

- **표준이 우리보다 잘게 나눈 자리** — 비골근지대·신근지대는 TA98에서 상·하 둘이다.
  하나를 골라 적으면 이 레코드가 아닌 것을 가리키게 된다.
- **우리가 더 잘게 나눈 자리** — 족저건막 세 밴드에 표준은 `aponeurosis plantaris`
  하나뿐이다. 셋에 같은 코드를 붙이면 그 코드가 셋 중 무엇도 가리키지 않는다.
- **표준이 더 넓은 자리** — 무릎 관절낭. `capsula articularis`는 있지만 그건
  모든 활막관절의 일반 용어이지 무릎의 것이 아니다. 이름이 같다고 붙이면 안 되는
  경우가 이쪽이다.
- **표준에 그 범주가 없는 자리** — 힘줄 12개. TA98의 하지 항목에서 `tendo`로
  시작하는 것은 `tendo calcaneus` 하나뿐이다. 표준은 근육을 이름 짓고 그 힘줄을
  따로 이름 짓지 않는데, 우리는 배와 힘줄을 나눈다. 만져지는 자리가 다르기
  때문이다 — 후경골근 배는 종아리 깊숙이 있고 그 힘줄은 안쪽 복사뼈 뒤에 있다.

넷 다 우리가 틀린 게 아니라 **단위가 다른** 것이고, 우리 단위는 손끝이다.

**상하지 동명 함정.** 손과 발은 근육 이름이 겹친다(소지외전근·단소지굴근·
배측골간근·충양근·종자골·굴근지대·신근지대). 하지 가지(`membri inferioris`) 밖의
코드는 전부 버린다. 신근지대가 실제로 걸린 자리다 — 하지 쪽은 이름에
`superius`/`inferius`가 붙어 있어서 짧은 이름으로 검색하면 **손목 것만 잡힌다.**
후보가 하나뿐이라는 건 맞다는 뜻이 아니다.

**대조가 우리 데이터의 오류를 하나 잡았다.** 무지내전근 사두·횡두는 `ko`와 `en`은
갈라져 있는데 `la`가 둘 다 `musculus adductor hallucis`, 즉 모근육 이름이었다.
비복근 두 갈래는 이미 `caput mediale musculi gastrocnemii`로 적고 있었으므로 우리
안에서도 일관되지 않았던 것이다. `caput obliquum/transversum musculi adductoris
hallucis`로 고쳤다. 밖에 기준을 두는 값어치가 여기 있다.

개발 중 무결성 검사가 코드 형태와 중복을 본다. 중복이 문제인 이유는 두 레코드가
같은 표준 항목을 가리킨다면 둘 중 하나는 그 항목이 아니기 때문이다.
...
분절 정합은 **자세**가 다른 자료를 다뤘다. 이건 **비율**이 다른 자료를 다룬다.
둘 다 "상사변환이 존재하지 않는다"에서 출발하지만 원인도 답도 다르다.

Gray437(종아리 앞면)과 앞 뷰를 재보니 발목으로 좁아지는 비율은 2.01 대 1.96으로
거의 같은데, 길이/최대폭이 **2.27 대 3.75**였다. 도판 쪽 다리가 훨씬 뭉툭하다.
등방 배율은 두 축을 함께 늘이므로 **폭을 맞추면 길이가, 길이를 맞추면 폭이**
그만큼 틀어진다. 비등방 배율은 각도를 바꾸므로 금지다.

**그리고 이번엔 자료가 옳다고 전제할 수 없다.** 실제 다리의 길이/폭이 3.5쯤이라
뷰(3.75)가 도판(2.27)보다 실제에 가깝다. 도판의 비율을 믿고 뷰를 고치면 더
틀리게 된다. 목판화가 세로로 눌린 것으로 보인다.

### 무엇을 옮기는가

도판에서 믿을 수 있는 것은 비율이 아니라 **내부 상대 관계**다. 그래서 좌표
대신 두 값을 옮긴다.

- `s` — 두 기준 높이 사이에서 몇 %인 높이인가
- `u` — 그 높이에서 폭의 어느 지점인가

기준 높이는 자료와 뷰에서 **같은 해부학적 자리**를 가리키는 둘만 쓴다(앞 뷰는
경골조면과 발목 최협부). 이게 이 방법의 유일한 판단 지점이고, 여기가 틀리면
나머지가 전부 조용히 틀어진다. 늘릴수록 근거가 아니라 손이 는다.
...
등각사상이 아니므로 자료의 원은 뷰에서 원이 아니다. **도형을 중심선 + 폭으로
저장한 것이 여기서 값을 한다** — 옮기는 양이 정확히 상대 위치와 폭 비율이라,
살아남는 것만 저장하고 있다. path 문자열이었다면 살아남지 않을 것을 옮기는
셈이 됐다.

정합의 rms에 해당하는 자리에는 오차 대신 **왜곡**을 보고한다(`distortion()`).
맞추는 게 아니라 일부러 늘이는 방법이라 "얼마나 잘 맞았나"가 성립하지 않는다.
앞 뷰의 비등방은 1.64다.

### 세 번째 등급 — `fidelity: 'normalized'`

무릎에서 "걸친 구조를 `traced`로 둘지는 **첫 실제 트레이싱 뒤에 정한다**"고
미뤄둔 질문의 답이 이것이다. 등급 체계를 미리 만들지 않은 이유가 여기서
드러난다 — 실제로 해보기 전에는 `traced`와 `schematic` 사이에 무엇이 들어갈지
알 수 없었다.

- `schematic` — 손으로 찍었다. 아무것도 주장하지 않는다.
- `normalized` — **상대 위치는 자료에서, 비율은 뷰에서.** 출처가 필수다.
- `traced` — 자료의 좌표를 상사변환으로 그대로 옮겼다. 출처가 필수다.

`normalized`를 `traced`라고 부르지 않는 이유는 하나다. 좌표를 옮긴 게 아니다.

고지 문구도 셋으로 갈린다. 등급이 섞인 뷰는 `mixed`이고, 가장 강한 등급으로
뭉뚱그리면 고지가 과장되고 가장 약한 등급으로 뭉뚱그리면 트레이싱한 층이
묻힌다.

**저작 도구는 여전히 `traced`만 만든다.** 정합이 `fitRegistration` 기반이라
실제로 상사변환을 쓰기 때문이고, 그게 맞다.
...
- Visible Korean, anatomy.co.kr (CC BY-NC) — 단면 + 분할 이미지. 좌표가 아니라
  `name.ko` 용어 검증용.
- **TA98 (Terminologia Anatomica 1998) — 이름 대조 기준.** `Structure.ta`.
  기계 판독본은 TAViewer(`mhalle/taviewer`, MIT)의 `src/human.json`을 썼다.
  개별 용어의 저작권은 FIPAT이 정리 중이고 PD로 공개할 방침을 밝혔으나 확정된
  것은 아니므로, **데이터셋을 레포에 넣지 않고 코드만 옮겨 적었다.**
- FMA — 온톨로지 ID (`fmaId` 연결). 위 데이터셋이 TA98 항목과 함께 담고 있다
