...
**대조 방법 (종아리 23개).** Open Anatomy Project의 TAViewer(`mhalle/taviewer`,
MIT)에 들어 있는 `src/human.json`이 TA98 코드 · 라틴어 · 영어 · FMA id를 트리로
담고 있다. 라틴어 이름만으로 기계 대조했고, 표준이 `m. soleus`처럼 줄여 쓰므로
`m.`·`n.`·`v.`·`a.`를 풀어 비교했다. 같은 이름이 상지에도 있는 굴근지대는
`musculi membri superioris` 아래 것을 빼고 하지 것만 남겼다. 22개가 붙었다.

**비우는 것도 결과다.** 대응 항목이 없으면 근처 코드를 끌어다 적지 않는다. 그러면
그게 나중에 근거로 둔갑한다 — 도판 번호를 캡션으로 확정하는 규칙과 같다. 종아리
23개 중 `fibular-retinaculum` 하나가 비어 있고, 이유는 표준이 이걸 상·하 둘로
나눠 싣기 때문이다. 우리는 눌러서 갈라낼 수 없는 한 덩이로 두었으므로 어느
코드도 그 레코드를 가리키지 않는다.

빈 자리는 세 종류로 갈린다. 표준이 우리보다 잘게 나눈 경우(비골근지대), 우리가
더 잘게 나눈 경우(비복근 두 갈래는 TA98에 근육 하나뿐이다), 그리고 근육 배와
힘줄을 따로 두는 우리 규칙에 표준이 대응하지 않는 경우다. 셋 다 우리가 틀린 게
아니라 **단위가 다른** 것이고, 우리 단위는 손끝이다.
